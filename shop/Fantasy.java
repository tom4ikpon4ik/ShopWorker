...
public class Fantasy{
    ...
    public static void getfantasy() throws SQLException{

      Connection conn = DAOConnection.sqlconnection();      //Verbindung mit der DB
    try {
      System.out.println("Connected to Fantasy");
      String query = "SELECT * FROM SHOP.FANTASY";            //Auswahl aus Produkt DB
      Statement st = conn.createStatement();
      ResultSet rs = st.executeQuery(query);          //Ausführen und Ergebnis bekommen

      /*Testeintrag erstellen - Modell wie in Registrierung der Kundennamen
      boolean ok = st.execute("insert into Fantasy(PRODUKT_ID,PRODUKT_NAME,PRODUKT_BESCHREIBUNG,PRODUKT_VORRAT,PRODUKT_PREIS) values ('','','','','')");
